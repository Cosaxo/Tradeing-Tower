...
// ---------------------------------------------------------------------------
// growThread (compounding gains into layers 1, 2, 3 — NOT layer 4)
// ---------------------------------------------------------------------------
...
describe("growThread", () => {
  ...
  it("returns lapLayerAdd matching the gain (layer 3)", () => {
    const { ttState, thread } = seed(200);
    const r = growThread({ ttState, threadId: thread.id, gain: 30 });
    expect(r.lapLayerAdd).toBe(30);
  });
  ...
  it("damage above buffer eats principal AND ttFace pro-rata", () => {
    let { ttState, thread } = seed(100);
    ttState = growThread({ ttState, threadId: thread.id, gain: 30 }).ttState;
    // principal 130, ttFace 100, buffer 30. Damage 50 → eats 30 buffer
    // + 20 of ttFace.
    const dmg = damageThread({ ttState, threadId: thread.id, delta: 50 });
    const t = dmg.ttState.threads[0];
    expect(t.principal).toBe(80);
    expect(t.ttFace).toBe(80); // matched to new principal
...
describe("findThreadByLapId", () => {
  it("returns the thread whose layer-3 LAP matches", () => {
    let s = initTtState();
    ...
    const t = findThreadByLapId(opened.ttState, "LAP-XYZ");
    expect(t?.id).toBe(opened.thread.id);
  });

  it("returns null for unknown lapId", () => {
    expect(findThreadByLapId(initTtState(), "missing")).toBeNull();
  });
});
